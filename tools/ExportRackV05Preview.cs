...
internal static class ExportRackV05Preview
{
    private const string OpenAssemblyFileName = "Rack4Modules_OpenCase_V05.SLDASM";
    private const string TransportAssemblyFileName = "Rack4Modules_TransportClosed_V05.SLDASM";
    private const string ClearanceAssemblyFileName = "Rack4Modules_ClearanceCheck_V05.SLDASM";
    private const string Tilt60AssemblyFileName = "Rack4Modules_DesktopTilt60_V05.SLDASM";
    private const string Tilt75AssemblyFileName = "Rack4Modules_DesktopTilt75_V05.SLDASM";
    ...
    private static int Main(string[] arguments)
    {
        ...
        try
        {
            ...
            string tilt75AssemblyPath = ProjectAssemblyPath(root, Tilt75AssemblyFileName);

            RequireFile(openAssemblyPath, "V0.5 open assembly");
            RequireFile(transportAssemblyPath, "V0.5 transport assembly");
            RequireFile(clearanceAssemblyPath, "V0.5 clearance-check assembly");
            RequireFile(tilt60AssemblyPath, "V0.5 60-degree desktop assembly");
            RequireFile(tilt75AssemblyPath, "V0.5 75-degree desktop assembly");
            ...
            Log("VERIFIED_NATIVE_ASSEMBLY=" + tilt75AssemblyPath);
            ...
            // Refuse before changing a view if any render target was already open and dirty.
            // Unrelated open documents are never activated, saved, rebuilt or closed.
            RequireExistingTargetIsSafe(application, openAssemblyPath, "V0.5 open assembly");
            RequireExistingTargetIsSafe(application, tilt60AssemblyPath, "V0.5 60-degree desktop assembly");
            RequireExistingTargetIsSafe(application, tilt75AssemblyPath, "V0.5 75-degree desktop assembly");

            openCase = OpenExactAssembly(application, openAssemblyPath, "V0.5 open assembly");
            tilt60 = OpenExactAssembly(application, tilt60AssemblyPath, "V0.5 60-degree desktop assembly");
            tilt75 = OpenExactAssembly(application, tilt75AssemblyPath, "V0.5 75-degree desktop assembly");
            ...
            Log("TILT75_SOURCE_ASSEMBLY=" + Path.GetFullPath(tilt75.Document.GetPathName()));
            ...
            ExportView(
                openCase.Document,
                exportsDirectory,
                "Rack4Modules_OpenCase_V05_01_ModuleFront.png",
            ...
            ExportView(
                openCase.Document,
                exportsDirectory,
                "Rack4Modules_OpenCase_V05_02_Isometric.png",
            ...
            ExportView(
                openCase.Document,
                exportsDirectory,
                "Rack4Modules_OpenCase_V05_03_UpperEdge_IO.png",
            ...
            ExportView(
                openCase.Document,
                exportsDirectory,
                "Rack4Modules_OpenCase_V05_04_Back_VESA.png",
            ...
            ExportView(
                tilt60.Document,
                exportsDirectory,
                "Rack4Modules_DesktopTilt60_V05_05_Isometric.png",
                swStandardViews_e.swIsometricView,
                "desktop-tilt-60-isometric");

            ActivateExactAssembly(application, openCase.Document, openAssemblyPath);

            // A deterministic cropped close-up would require a model-specific camera or view transform.
            // Until such a camera is released with the assembly, export the complete standard right view
            // instead of pretending that a zoom-to-fit image is a precise close-up.
            ExportView(
                openCase.Document,
                exportsDirectory,
                "Rack4Modules_OpenCase_V05_06_RightSide_StandStorage.png",
                swStandardViews_e.swRightView,
                "open-case-right-side-full-standard-view-stand-storage");
            Log(
                "RIGHT_SIDE_STAND_STORAGE_VIEW=full-right-standard-view; requested=close-up; " +
                "crop=not-applied; reason=no-released-model-specific-camera");

            ActivateExactAssembly(application, tilt75.Document, tilt75AssemblyPath);
            ExportView(
                tilt75.Document,
                exportsDirectory,
                "Rack4Modules_DesktopTilt75_V05_07_Isometric.png",
                swStandardViews_e.swIsometricView,
                "desktop-tilt-75-isometric");
        ...
        {
            // Close only V0.5 documents that this exporter opened. Never close a document
            // that was already open when the exporter attached, and never discard a document
            // that became dirty while the exporter was running.
            CloseOwnedOtherDocument(application, tilt75, "V0.5 75-degree desktop assembly", ref exportFailure);
            CloseOwnedOtherDocument(application, openCase, "V0.5 open assembly", ref exportFailure);

            if (application != null && !string.IsNullOrEmpty(tilt60AssemblyPath))
            {
                try
                {
                    if (tilt60 == null && File.Exists(tilt60AssemblyPath))
                    {
                        tilt60 = OpenExactAssembly(
                            application,
                            tilt60AssemblyPath,
                            "V0.5 60-degree desktop assembly");
                    }
                ...
                catch (Exception restoreException)
                {
                    RecordSecondaryFailure(
                        ref exportFailure,
                        "Preview cleanup completed, but the DesktopTilt60 V0.5 interface could not be restored.",
                        restoreException);
                }
            }
        }
        ...
        string description)
    {
        ...
        if (existing.GetSaveFlag())
        {
            throw new InvalidOperationException(
                ...
                "Refusing to activate it, change its view, save it or close it: " +
        ...
        string description)
    {
        ...
        catch
        {
            if (openedByThisRun && document != null)
            {
                ...
                {
                    // Preserve the original open/validation failure.
                }
            }
        ...
        string viewLabel)
    {
        ...
        string ownershipToken = ".rack4modules-v05-preview-" +
        ...
        string tilt60AssemblyPath)
    {
        ...
        {
            throw new InvalidOperationException(
                "DesktopTilt60 V0.5 has unsaved changes. Refusing to alter its final view.");
        }
        ...
        string ownershipToken)
    {
        ...
        if (!string.Equals(fileName, ownershipToken + ".bmp", StringComparison.Ordinal) ||
            !ownershipToken.StartsWith(".rack4modules-v05-preview-", StringComparison.Ordinal))
        {
            throw new InvalidOperationException(
    ...
    {
        Console.WriteLine("[RackV05Preview] " + message);
        Console.Out.Flush();
    }
